...
## Phase 16 — Plugin system (self-extension)

Goal: **plugins** are installable/uninstallable extensions of three kinds — **MCP tool extensions**, **API endpoints**, and **UI components** — that LLM agents (Claude Code, MiniMax, future `AgentProvider`s) can **create, propose, and use entirely over `/mcp`**, with a mandatory human approval gate before anything executes. This is the platform's **self-improvement mechanism**: an agent identifies a missing capability, scaffolds + implements + tests a plugin over MCP, the human approves it in the portal, and the capability is live for every future agent run and for the UI.

Vision: "Baitler grows its own abilities." The compounding loop is *agent authors → human approves → everyone (agents + UI) uses*. Every gate the platform already has is reused: the `review="draft"` queue, the `activity` provenance log with `run_id`, the `X-Baitler-Run` protocol-layer refusal (`mcp/mod.rs`), the opaque-origin no-cookie serving precedent (`/p/*`), `kn_link`/`knowledge_search` (a plugin is a first-class knowledge item), and the run **butler report** (a run that drafted a plugin shows it with a one-click review jump). The **one genuinely new primitive** is a memory-isolated WASM sandbox — the first untrusted-code-execution boundary in the codebase.

Architecture: a plugin is one row in a new `plugin` table plus one self-contained, content-hashed **bundle**: `manifest.json`, an optional `plugin.wasm` (server logic; UI-only plugins ship none), and optional `ui/` static assets. One plugin may declare several capabilities of mixed kinds via the manifest's `tools[]` / `endpoints[]` / `ui[]` arrays, each binding to a WASM `export` or a UI asset. **Decisions taken up front:** (a) **Runtime = Extism (Wasmtime substrate), embedded in the single `baitler-api` binary** — in-process (no daemon), default-deny host-controlled capability manifest, wall-clock kill via epoch interruption + memory cap per instance, and the best LLM-authoring DX of the WASM options (an agent writes TypeScript/JS via the QuickJS PDK — or Rust/Python — and compiles to one `.wasm`). Embedded JS engines (rquickjs/boa) are rejected for untrusted code (no real sandbox); raw Wasmtime/WIT is deferred until portability demands it. The dependency is **heavy** (multi-minute cold compile, larger binary), so the whole feature is gated behind `PLUGINS_ENABLED=false` (fail-fast `parse_bool`, mirroring `MCP_ENABLED`) **and** a Cargo feature flag so non-plugin builds stay lean. (b) **Capabilities are default-deny and there is NO filesystem capability, ever** — the manifest has no `fs`/`allowed_paths` field, and host functions are a **closed enum** of owner-scoped wrappers over existing repos (`kn_search`, `ideas_get/create`, `pages_create`, `plugin_kv_get/set`, `log`, …) — eliminating the surface, not trusting reviewers to never approve it; the `workspace.rs` jail is untouched. (c) **Plugins never define tables** (SCHEMAFULL + the one-transaction migration runner forbid runtime DDL); they persist via `plugin_kv` host fns, keyed on `(owner, slug, k)` — *slug*, not plugin uuid, so KV survives version upgrades. (d) **`status` is distinct from `review`**: `review` is the human gate, `status` (`draft|approved|enabled|disabled|rejected`) is the operational state; `plugins_create` **hard-forces both to draft regardless of args** (`clean_review` accepts `review="published"` — verified `tools.rs` — so plugins get a stricter rule than reuse). (e) **No approve/enable verb exists on the MCP surface at all** — approval/enable/disable are portal-only REST; removing the verb is a stronger guarantee than a per-call refusal. (f) **Plugin MCP tools are namespaced `plugin__{slug}__{tool}`** and advertised by concatenation: the drift guard is scoped to a renamed `static_definitions()` (the hand-written `known[]` + count assert is preserved verbatim, plus a new assert that no static name starts with `plugin__`), while dynamic tools need zero lockstep edits. (g) **Upgrade = new row, same slug, higher `version_int`, re-enters draft→review** (grant changes are re-reviewed); install pins `bundle_sha256`, re-checked on every load; the prior row is kept for rollback. (h) **A host plugin-ABI epoch (`HOST_PLUGIN_API_VERSION`)** guards platform evolution: on registry load, any enabled plugin with an older `api_version` is auto-disabled (logged + surfaced in the portal) rather than silently breaking after a platform migration.

```jsonc
// manifest.json — validated against plugins::manifest::Manifest (deny unknown fields)
{ "name": "csv-stats", "version": "0.3.1", "api_version": 1,
  "tools":     [{ "name": "csv_summarize", "export": "summarize", "description": "…",
                  "input_schema": { "type": "object", "properties": {}, "required": [] } }],
  "endpoints": [{ "method": "POST", "path": "summarize", "export": "summarize" }],
  "ui":        [{ "slot": "object|rail|butler_widget|detail", "key": "csvtool",
                  "label": "CSV", "icon": "table", "entry": "ui/index.html" }],
  "capabilities": {                       // DEFAULT-DENY — absent = denied
    "host_fns": ["kn_search", "plugin_kv_get", "plugin_kv_set"],
    "egress":   [],                       // allowlist; ALWAYS empty in v1 (unwired)
    "secrets":  [],                       // names resolved via the Phase-6 encrypted store
    "memory_max_pages": 256, "timeout_ms": 2000 } }  // no fs field exists, ever
```

### Security reality (read before B) — agent-authored code is hostile until approved

- **The escalation question is the decisive test.** The run refusal at `mcp/mod.rs` is a protocol-layer check *before* dispatch; plugin tools open a new dispatch path it doesn't cover. Closed at two independent points: (1) **no grantable disk surface exists** (decision (b)) — a restricted `X-Baitler-Run` run can never regain workspace access via a plugin, even an approved one; (2) the **actor threads into plugin dispatch** — `tools::call` widens from `owner: &str` to `&Actor` (mechanical refactor, every arm reads `actor.owner`) so host fns re-derive scope, plugin rows store `author_agent`/`run_id`, and runs are **refused `plugins_uninstall`** (destructive; mirrors the `workspace_` refusal). Runs *may* author/validate/test/create (forced draft — proposing is the point) and *may* invoke **enabled** plugins (human-approved capability; host fns grant nothing a run can't already do via `ideas_create` et al.).
- **Five convergent gates**: default-deny manifest → mandatory human approval (forced draft + portal-only approve, surfaced by **extending `review_queue` in `knowledge/repo.rs` to a `plugins` section** — it is hardwired to idea+document today) → provenance (`bundle_sha256` pinning re-verified on load; `activity` rows with `run_id`) → sandboxed substrate (Extism per-invocation fresh store; traps → typed `AppError`, never internals) → no ambient escalation (runs propose, humans enable).
- **Host-fn writes log their own provenance.** `activity::entry_for` is keyed on the MCP tool name, so a plugin calling `ideas_create` *through a host fn* would bypass it — host fns therefore call `activity::record` directly with the plugin slug as agent + the invoking `run_id`. Every `plugin__*` invocation also logs a `plugin.invoke` row (generic `entry_for` arm).
- **UI plugins never touch the cookie origin.** Assets serve from a new outside-CORS route `GET /plugin-ui/{uuid}/{*path}` (uuid, not slug — slugs are only unique per owner and the route is public-style; mirrors the `/p/*` mount in `routes/mod.rs`) under CSP `default-src 'none'; script-src 'self'; style-src 'unsafe-inline'; sandbox allow-scripts` — JS runs (a justified step up from the zero-JS pages CSP) but **no `allow-same-origin`**: an opaque, cookieless origin. The host bridge is an origin-pinned, typed postMessage RPC relayed through the credentialed `apiFetch` — the frame never holds credentials.
- **Endpoint plugins are time-boxed at the route layer** (`tokio::time::timeout` → 504), not just inside WASM — a hung host-fn DB call must not pin an Axum worker. Returned HTML passes `convert::sanitize` (+ `harden_for_render` on any server-side render path).
- **Secrets**: a declared secret *name* resolves through the Phase-6 per-owner encrypted key store (`crypto.rs`) into an Extism config var; plugins never see `APP_SECRET`, ciphertext, or another owner's keys. Reviewers check secret handling at approve time (an approved plugin that leaks its own secret is an approved-code flaw, same trust level as any approved code).
- **Size truth**: the MCP body cap is `MCP_MAX_BODY_BYTES = 32 MiB` (`routes/mod.rs`), so a base64 bundle arg tops out around ~20 MiB of artifact — fine for v1 (a TS-PDK `.wasm` is hundreds of KB); a chunked/HTTP upload path is deferred until a real bundle needs it.
...
> **Milestone B shipped** (branch `phase-16-plugins`). Migration `0018_plugins.surql` (`plugin` + `plugin_kv`,
> SEARCH indexes over name/description); `plugin` first-class in `ITEM_TYPES`/`MEMBER_TYPES`/`ProjectMembers`/
> `MemberCounts`/`knowledge_search`/`review_queue` (+ optional `plugins` on the frontend `ReviewQueue` type);
> `src/plugins/{manifest,model,repo,registry,routes}.rs` always compiled, `{runtime,hostfns}.rs` behind the
> `plugins` Cargo feature (`extism` optional dep); 9 `plugins_*` meta-tools — **80→89**, drift guard moved;
> `plugins_uninstall` refused for run actors at the protocol layer; portal-only REST lifecycle with enable
> hot-load (sha256 + ABI re-verified; load failure rolls back to `disabled`); host fns record their own
> activity rows (`plugin:{slug}` + invoking `run_id`); `plugin.invoke` logged on **both** execution paths.
> **As-built deltas:** (a) manifest stored as one validated JSON **string** body (mindmap-graph convention),
> not FLEXIBLE objects — `capabilities` lives inside it; `ui_assets` deferred to 16.C's migration; (b) upgrade
> = **same row** re-drafted via `plugins_create {replace:true}` (`version_int` bump) rather than a second row
> — the (owner,slug) UNIQUE index stays simple and rollback-to-prior-version moves to 16.D; (c) `plugins_test`
> failures return as **data** (`{ok:false,error,ms,limits_hit}`) and content-write host fns refuse in the
> throwaway sandbox (in-memory kv) — `logs` capture deferred (guest `log` goes to tracing); (d) per-run
> rate-limiting of `plugins_test` deferred (timeout/memory caps bound the cost); telemetry = per-invocation
> tracing fields (slug/export/ms/limits_hit), not a counter module; (e) egress/`secrets` manifest fields exist
> but are **validation-rejected** in v1 (not merely unwired); (f) `wat`-compiled fixtures (echo/spin/kv) stand
> in for the PDK fixture, also proving hand-written-WASM hostility cases (ungranted import → instantiation
> failure; infinite loop → epoch kill). **Verified green:** fmt + clippy `-D warnings` + tests on BOTH configs
> (198 default / 204 with `--features plugins`); frontend tsc + 82 tests; live binary probe of the whole loop
> (89 tools, forced draft, approve/enable, dynamic `plugin__live-probe__run` in tools/list, runtime dispatch,
> run-refusal, uninstall); CI extended with `clippy`/`test --features plugins` steps; `docs/plugins.md` written.
> **Adversarial 4-lens review** (sandbox/correctness/conventions/agent-DX; 25 findings → 10 confirmed, 15
> refuted), all fixed + regression-tested: a rejected plugin no longer haunts the review queue (`drafts_of`
> gains a plugin `status="draft"` guard); a run cannot `replace` an approved/enabled plugin (containment —
> bundle-swap-under-a-trusted-slug + unload was a reachable denial-of-capability); `replace` is name-stable
> (refuses a non-existent slug instead of silently creating a duplicate + orphaning kv);
> `looks_like_limit` anchored to Extism's exact kill strings (guest text is attacker-authored); the scaffold's
> emitted build command/filename/host-fn example were all wrong-on-arrival for a blind agent (bogus npm
> package, mismatched filename, undeclared identifier) — now consistent with the real Extism JS PDK;
> `plugins_get/export/uninstall` accept slug-or-id; missing-export failures list the declared exports.

- [x] **16.2** Migration `0018_plugins.surql` (SCHEMAFULL, mirroring `0010_pages.surql`): `plugin` — `uuid` (UNIQUE), `owner`, `name`, `slug` (UNIQUE ON owner,slug via `src/slug.rs`), `version`/`version_int`/`api_version`, `manifest` + `capabilities` (FLEXIBLE objects), `bundle_sha256`, `wasm_b64`, `ui_assets`, `kinds`, `status` DEFAULT "draft", `review` DEFAULT "draft", `author_agent?`, `run_id?`, timestamps, index ON owner,status — plus `plugin_kv` (`owner`,`slug`,`k`,`v` FLEXIBLE, UNIQUE ON owner,slug,k) and a `plugin` `kn_text` SEARCH section over name+description (11.1-gated, `CONTAINS` fallback). Add `"plugin"` to `ITEM_TYPES`/`MEMBER_TYPES` and one typed section in `knowledge::repo::search`.
- [x] **16.3** New `src/plugins/{mod,manifest,repo}.rs`: deny-unknown-fields `Manifest` validation; owner-scoped CRUD (`PLUGIN_SELECT`); `bundle_sha256` computed on create; lifecycle transitions incl. `rejected` (terminal; purgeable); **`plugins_create` hard-forces `review`/`status="draft"`**, ignoring caller-supplied review; delete → `scrub_item_links` + cascade `plugin_kv` + registry unload. **Extend `review_queue` (`knowledge/repo.rs`) + the `ReviewQueue` DTO/portal types with a `plugins` section.**
- [x] **16.4** Extism runtime `src/plugins/{runtime,hostfns,registry}.rs` (behind the `plugins` feature + `PLUGINS_ENABLED`): `PluginRegistry::{load, dispatch, tool_defs, unload}` — `load` builds the Extism manifest from `capabilities` ∩ host ceiling, verifies `bundle_sha256`, auto-disables on `api_version` < `HOST_PLUGIN_API_VERSION`; populate in `lib.rs::build_state` after `migrations::run` (next to `fail_orphaned_runs`) from `status="enabled"` rows. `hostfns.rs` = the closed enum of owner-scoped wrappers, **each recording its own `activity` row** (plugin slug as agent + invoking `run_id`); per-invocation fresh store; `timeout_ms` epoch kill + `memory_max_pages`; traps → typed error. A per-slug invocation/trap/limit counter in `telemetry.rs`.
- [x] **16.5** MCP meta-tools (lockstep: `call` arms + `static_definitions()` + `known[]` + count + `entry_for`): `plugins_scaffold` (manifest + PDK skeleton + host-fn type stubs; pure helper), `plugins_validate` ({manifest, source} → structured `{errors:[{path,msg,hint}]}`), `plugins_test` (one export in a **throwaway** sandbox → `{output, logs, ms, limits_hit}`; rate-limited per `run_id`; no persistence), `plugins_create`, `plugins_list`/`plugins_get`, `plugins_invoke` (enabled plugins only; also reachable as `plugin__{slug}__{tool}`), `plugins_export` ({manifest, wasm_b64, ui_b64, sha256} — backup/transfer), `plugins_uninstall` (**refused for `X-Baitler-Run` actors**, mirroring the `workspace_` gate) — 9 tools, **80→89**. `entry_for` arms: `plugin.create`/`plugin.uninstall` + generic `plugin__*` → `plugin.invoke`. `initialize` keeps `listChanged` honest: there is **no server→client channel on the JSON-response transport** (`GET /mcp` → 405) and Claude Code registers no handler for the notification — **new tools appear on the next MCP session/run**, acceptable because enable is human-gated and inherently async; document it, build no SSE.
- [x] **16.6** Portal-only lifecycle REST (credentialed tree): `POST /plugins/{id}/approve` (status=approved, review=published), `/enable` (registry hot-load), `/disable` (unload, row kept), `/reject`; `GET /plugins`, `GET /plugins/{id}`. **No approve/enable over MCP.**
- [x] **16.7 (tests-with-features)** Backend integration test driving the loop over JSON-RPC with a fixed `.wasm` fixture (no egress, no toolchain): scaffold → validate → test → create (asserts forced draft) → portal approve+enable → `tools/list` shows `plugin__…` → invoke returns output → uninstall scrubs links + KV. Assert: a run actor is refused `plugins_uninstall` and has no enable path; host-fn writes produce attributed `activity` rows carrying `run_id`; digest mismatch refuses load; stale `api_version` auto-disables; drift guard green over statics. `docs/plugins.md` (authoring guide, manifest reference, capability model, next-session visibility). **Ships the complete create→approve→use loop.**
...
- [ ] **16.11** Per-owner install isolation + per-capability (not per-plugin) approval once auth lands; **network egress** wired (the `egress` allowlist with private-range/IP-literal SSRF denial at the host-fn layer); plugin signing beyond sha256; a marketplace / registry-URL install; server-side TS→wasm compile service (so restricted runs can author end-to-end without a host toolchain); and — only on verified client support — an SSE GET stream enabling live `tools/list_changed`.

> Acceptance: the **proving slice** works end-to-end over `/mcp` with no human in the authoring loop and a mandatory human in the approval loop — an agent (Claude Code or MiniMax run, or an external MCP client) scaffolds, validates, tests, and submits a plugin that lands as a **forced draft**, visible in the portal Review queue **and** the originating run's butler report; the human inspects the capability manifest + source and approves/enables; `tools/list` then advertises `plugin__{slug}__{tool}` (next session), `plugins_invoke` executes it inside Extism under its declared `host_fns`/memory/timeout only, every invocation and host-fn write logs an attributed `activity` row with `run_id`, and uninstall scrubs links + KV. A restricted run can author and propose but can reach **no** enable path and cannot uninstall; no plugin can touch the filesystem (no such capability exists); UI plugins render only in the opaque-origin iframe; the drift guard still pins the static catalog (89) and rejects `plugin__`-prefixed statics. Backend `build`/`clippy -D warnings`/`test`/`fmt` + frontend `tsc`/`eslint`/`vitest`/`build` green with `PLUGINS_ENABLED=false` *and* `=true` (fixture `.wasm`, no egress).
>
> **Deferred (build on real need):** egress + secrets actually wired into v1 plugins (manifest fields exist, deny-all); plugin-defined DB tables (never — `plugin_kv` is the contract); live `tools/list_changed` push; module-federation/runtime-ESM UI (host-privileged — never for agent code); plugin→plugin calls; non-WASM plugins; signing/marketplace/cross-owner sharing (16.11); mobile surfaces for UI-kind plugins (server kinds work for mobile via API/MCP from day one; UI kinds are web-only in v1).
>
> **Dependencies & sequencing:** builds on Phase 7.5/11 (MCP + Actor + activity + review), Phase 12 (the outside-CORS opaque-origin serving precedent), Phase 13 (runs, loopback MCP, butler report), and Phase 6 (encrypted key store, for `secrets` later). 16.A is a pure refactor and can land any time; 16.B is the proving slice and the bulk of the new machinery; 16.C reuses the same table/runtime/gate; 16.D waits on Phase 2 auth. Ships against the `DEV_OWNER` stub like every phase since 4.
 ...
 **Phase 16 (Plugin system)** is the self-improvement layer: it builds on Phase 7.5/11 (MCP, Actor, activity, review
queue), Phase 12 (the opaque-origin serving precedent), and Phase 13 (runs + butler report), and introduces the
codebase's first untrusted-code sandbox (Extism/Wasmtime, gated behind `PLUGINS_ENABLED` + a Cargo feature). Milestone
A (the registry seam) is a zero-behavior refactor that can land any time after Phase 15; B (MCP-tool kind) is the
proving slice for the create→approve→use loop; C (endpoint + UI kinds) reuses the same machinery; D waits on Phase 2.
Approval/enable are deliberately portal-only — no MCP verb exists — so agents propose and humans gate, regardless of
auth phase ordering.
